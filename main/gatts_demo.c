...
static bool sleep_bypass_mode = false;
...
void inactivity_timer_callback(TimerHandle_t xTimer) {
    if (sleep_bypass_mode) {
        ESP_LOGI(GATTS_TAG, "Sleep bypass mode active. Skipping sleep.");
        reset_inactivity_timer();  // restart timer
        return;
    }

    ESP_LOGI(GATTS_TAG, "Inactivity timeout reached. Entering light sleep.");

    uart_wait_tx_done(UART_NUM, pdMS_TO_TICKS(100));

    gpio_wakeup_enable(UART_RXD_PIN, GPIO_INTR_LOW_LEVEL);     
    esp_sleep_enable_gpio_wakeup();

    ESP_LOGI(GATTS_TAG, "Entering light sleep now...");
    esp_light_sleep_start();  

    ESP_LOGI(GATTS_TAG, "Woke up from light sleep.");
...
void uart_ble_forward_task(void *arg)
{
    ...
    while (1) {
        ...
        if (len > 0) {
            reset_inactivity_timer();
            data[len] = '\0';  // Null-terminate for string ops

            ESP_LOGI("UART", "Received: %s", data);

            // --- Command parsing ---
            if (strncmp((char *)data, "BLEON", 5) == 0) {
                ESP_LOGI(GATTS_TAG, "BLEON command received");
                esp_ble_gap_start_advertising(&adv_params);
            }
            else if (strncmp((char *)data, "BLEOFF", 6) == 0) {
                ESP_LOGI(GATTS_TAG, "BLEOFF command received");
                esp_ble_gap_stop_advertising();
            }
            else if (strncmp((char *)data, "STATUS", 6) == 0) {
                ESP_LOGI(GATTS_TAG, "STATUS command received");
                ESP_LOGI(GATTS_TAG, "BLE Connected: %s", ble_connected ? "Yes" : "No");
                ESP_LOGI(GATTS_TAG, "Sleep Bypass Mode: %s", sleep_bypass_mode ? "Enabled" : "Disabled");
            }
            else if (strncmp((char *)data, "BYPASS", 6) == 0) {
                ESP_LOGI(GATTS_TAG, "BYPASS command received");
                sleep_bypass_mode = true;
            }
            else if (strncmp((char *)data, "ENABLE", 6) == 0) {
                ESP_LOGI(GATTS_TAG, "ENABLE command received");
                sleep_bypass_mode = false;
                reset_inactivity_timer();
            }
            else {
...
inactivity_timer = xTimerCreate("inactivity_timer", pdMS_TO_TICKS(INACTIVITY_TIMEOUT_MS), pdFALSE, NULL, inactivity_timer_callback);
if (inactivity_timer) {
    xTimerStart(inactivity_timer, 0);
}


// UART config at usart0 same of prg one
...
xTaskCreate(uart_ble_forward_task, "uart_ble_forward_task", 4096, NULL, 10, NULL);
